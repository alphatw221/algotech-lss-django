...
class CampaignViewSet(viewsets.ModelViewSet):
    ...
    def list_campaign(self, request):
        api_user, key_word, campaign_status, order_by = getparams(request, ("key_word", "status", "order_by"),
    ...
    def update_campaign(self, request, pk=None):
        ...
        for platform, func in campaign_platform_dict.items():
            platform_campaign = func.copy()
            platform_campaign.update(json_data.get(platform, {}))
            json_data[platform] = platform_campaign
    ...
    def seller_total_revenue(self, request):
        ...
        if is_user:
            if int(comment_id) == 0:
                ...
                last_time = db.api_campaign_comment.find_one(
                    {'campaign_id': int(campaign_id), 'id': comment_id, 'platform': 'instagram'})['created_time']
                comment_datas = db.api_campaign_comment.find(
                    {'campaign_id': int(campaign_id), 'created_time': {'$gt': last_time}, 'platform': 'instagram'})
                comments_list = self.__create_comments_list(comment_datas)
    ...
    def comment_category_summarize(self, request, pk):
        ...
        for category in categories:
            comment_count = db.api_campaign_comment.find(
                {"campaign_id": int(pk), "categories": {"$elemMatch": {'$eq': category}}}).count()
            ret.append({"name": category, "comment_count": comment_count, "total_comment_count": total_comment_count})

        return Response(ret, status=status.HTTP_200_OK)

    @action(detail=True, methods=['GET'], url_path=r'comments/category/(?P<category_name>[^/.]+)/list',
            permission_classes=(IsAuthenticated,))
    @api_error_handler
    def comment_category_list(self, request, pk, category_name):
        ...
        comments = db.api_campaign_comment.find(
            {"campaign_id": int(pk), "categories": {"$elemMatch": {'$eq': category_name}}}, {"_id": 0})
