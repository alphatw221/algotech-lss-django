...
def comment_responding(platform_name, platform, pre_order, comment, campaign_product, qty, state):
    if platform_name == 'facebook':
        ...
        shopping_cart_info, info_in_pm_notice = i18n_get_additional_text(
            lang=platform['lang'])
        api_fb_post_page_comment_on_comment(platform['token'], comment['id'], text+shopping_cart_info)
        api_fb_post_page_message_on_comment(platform['token'], comment['id'], text+info_in_pm_notice)
    elif platform_name == 'youtube':
        return
    elif platform_name == 'instagram':
        return
